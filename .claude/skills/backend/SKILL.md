...
5. `fetch_listings` → for each requested card, `_fetch_one` paginates ALL live US listings (size 50, retries w/ backoff on 429/5xx). `_normalize_listings` language-filters and flattens. Result stored in module-global `_cached_card_data`.
6. `optimize_filtered` → applies dynamic filters with fallback tiers, then runs `optimizer.optimize`. Returns both the bundled `cart` and a baseline `firstCart` (cheapest listing per card).
...
## Caching
Two layers keyed by string (`filter_sets:<game>`, `fetch_cards:<game>:<set>`): in-memory `_mem_cache` + JSON files under `~/.masterset/cache/`, TTL 7 days. Listings live only in the in-memory `_cached_card_data` (not disk) and reset on restart — `check_listings_cache` lets the frontend fast-resume or trigger a re-scrape.
